...
namespace DAL
{
    public class DALBranch
    {
        ...
        public string GetAllBranchPaging(Paging Page)
        {
            string result = "";
            ...
            {
                DataSet ds = dbCon.ExecuteFetchCommandParameterized("SpBranchGetAllPaging", CommandType.StoredProcedure, ht);
        ...
        public string GetBranchForEdit(Paging Page)
        {
            string result = "";
            Hashtable ht = new Hashtable();
            //ht.Add("@PageNumber", Page.PageNo);
            ...
            if (dbCon.OpenConnection(dbFact.ConnectionString))
            {
                DataSet ds = dbCon.ExecuteFetchCommandParameterized("SpBranchByID", CommandType.StoredProcedure, ht);
                //if (ds.Tables[0].Rows.Count > 0)
                //{
                //    // success
                //    result = "<xml>" + String.Format("<Response><Code>{0}</Code><Message>{1}</Message></Response>{2}", Constants.SUCCESS, "", ds.GetXml()) + "</xml>";
        ...
        public SystemResponce AddBranch(Branch ObjComp)
        {
            SystemResponce result = new SystemResponce();
            Hashtable ht = new Hashtable();

            ht.Add("@CompanyID", ObjComp.CompanyID);
            ht.Add("@Code", ObjComp.Code);
            ht.Add("@Address", ObjComp.Address);
            ht.Add("@Ph1", ObjComp.Ph1);
            ht.Add("@Ph2", ObjComp.Ph2);
            ht.Add("@Email", ObjComp.Email);
            ht.Add("@WebSite", ObjComp.WebSite);
            ht.Add("@PostalCode", ObjComp.PostalCode);
            ht.Add("@Country", ObjComp.Country);
            ht.Add("@State", ObjComp.State);
            ht.Add("@City", ObjComp.City);
            ht.Add("@Status", ObjComp.Status);
            ...
            if (dbCon.OpenConnection(dbFact.ConnectionString))
            {
                int res = (int)dbCon.ExecuteScalarParameterized("SpBranchAddNew", CommandType.StoredProcedure, ht);
                if (res == -1)
                {
        ...
        public SystemResponce EditSaveBranch(Branch ObjComp)
        {
            SystemResponce result = new SystemResponce();
            Hashtable ht = new Hashtable();
            ...
            if (dbCon.OpenConnection(dbFact.ConnectionString))
            {
                int res = (int)dbCon.ExecuteScalarParameterized("SpBranchEditSave", CommandType.StoredProcedure, ht);
                if (res == 1)
                {
                    result.ErrorCode = Constants.SUCCESS;
