   ...
   public class DeductionDAL
    {
        ...
        public string GetAllDeduction()
        {
            ...
            {
                DataSet ds = dbCon.ExecuteFetchCommand("SpDeductionGetAll", CommandType.StoredProcedure);
        ...
        public SystemResponce DeductionAdd(Deductions objddct)
        {
            ...
            if (dbCon.OpenConnection(dbFact.ConnectionString))
            {
                int res = Convert.ToInt32(dbCon.ExecuteScalarParameterized("[dbo].[SpDeductionAddNew]", CommandType.StoredProcedure, ht));
                if (res == -1)
                {
        ...
        public string GetDeductionForPaging(Paging Page)
        {
            ...
            if (dbCon.OpenConnection(dbFact.ConnectionString))
            {
                DataSet ds = dbCon.ExecuteFetchCommandParameterized("SpDeductionGetAllPaging", CommandType.StoredProcedure, ht);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    // success
                    result = "<xml>" + String.Format("<Response><Code>{0}</Code><Message>{1}</Message></Response>{2}", Constants.SUCCESS, "", ds.GetXml()) + "</xml>";
        ...
        public SystemResponce DeleteDeduction(Deductions objddct)
        {
            SystemResponce result = new SystemResponce();
            Hashtable ht = new Hashtable();

            ht.Add("@DeductionID", objddct.ID);
            ...
            if (dbCon.OpenConnection(dbFact.ConnectionString))
            {
                int res = (int)dbCon.ExecuteScalarParameterized("SpDeductionDelete", CommandType.StoredProcedure, ht);
                if (res == -1)
                {
                    result.ErrorCode = Constants.AlreadExists;
                    result.Msg = Constants.MessageAlreadExists;
        ...
        public SystemResponce EditDeduction(Deductions objddct)
        {
            SystemResponce result = new SystemResponce();
            Hashtable ht = new Hashtable();
            ...
            if (dbCon.OpenConnection(dbFact.ConnectionString))
            {
                ...
                int res = (int)dbCon.ExecuteScalarParameterized("SpDeductionEdit", CommandType.StoredProcedure, ht);
                if (res == -1)
                {
                    result.ErrorCode = Constants.AlreadExists;
                    result.Msg = Constants.MessageAlreadExists;
        ...
        public string GetDeductionByID(Paging Page)
        {
            ...
            if (dbCon.OpenConnection(dbFact.ConnectionString))
            {
                DataSet ds = dbCon.ExecuteFetchCommandParameterized("SpDeductionByID", CommandType.StoredProcedure, ht);
                //if (ds.Tables[0].Rows.Count > 0)
                //{
                //    // success
                //    result = "<xml>" + String.Format("<Response><Code>{0}</Code><Message>{1}</Message></Response>{2}", Constants.SUCCESS, "", ds.GetXml()) + "</xml>";
